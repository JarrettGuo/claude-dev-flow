...
[ "${FLOW_LOG_STDERR:-0}" = "1" ] && printf "[%s] ⏸ GATE 等待用户确认问题描述\n" "$TS" >&2
```
...
[ "${FLOW_LOG_STDERR:-0}" = "1" ] && printf "[%s] ⏸ GATE 等待用户确认根因分析\n" "$TS" >&2
```
...
[ "${FLOW_LOG_STDERR:-0}" = "1" ] && printf "[%s] ↻ RETRY Verify 失败，回到 Phase 4\n" "$TS" >&2
```
...
[ "${FLOW_LOG_STDERR:-0}" = "1" ] && printf "[%s] ✓ COMPLETE /fix 流程完成\n" "$TS" >&2
